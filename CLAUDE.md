...
branch is **webos-2.6.1**, based on ScummVM v2.6.1.
...
- **webos-2.5**: based on ScummVM v2.5.0 tag — working. 67 plugins, confirmed
  on HP TouchPad.
- **webos-2.6.1**: based on ScummVM v2.6.1 tag — current branch. 69 plugins
  (new in 2.6: asylum, chewy, hypno, nancy + others), tested working on HP
  TouchPad. Required disabling MT32EMU (C99 long-double math absent in glibc 2.5).
...
| Build directory | `/home/jonwise/Projects/scummvm-webos261-build` |
| WebOS PDK | `/opt/PalmPDK` |
| WebOS SDK | `/opt/PalmSDK/0.2` |
| Linaro GCC 4.9.4 | `~/Projects/qupzilla/toolchains/gcc-linaro/bin/` |
| IPK output | `scummvm-webos261-build/portdist/org.scummvm.scummvm_*.ipk` |
...
**Works (tested on HP TouchPad with webos-2.6.1 / ScummVM 2.6.1):**
- 69 engine plugins load and work (all from the v2.6.1 engine set)
...
- MT32EMU: disabled — v2.6.x MT32 code uses C99 long-double math (acoshl,
  at_quick_exit, etc.) absent in WebOS PDK glibc 2.5 sysroot. Linaro 4.9.4
  libstdc++ cmath/cstdlib declare `using ::acoshl` etc. which fail to compile.
  Add `--enable-mt32emu` with a PDK-sysroot-compiled libm to restore.
